...
module Head = struct
  ...
  let map f hd = match hd with
    ...
    | Fun (x, tele, body) -> Fun (x, List.map (fun (v, ty, t) -> (v, f ty, Option.map f t)) tele, f body)
    | Type u -> Type u
...
let rec of_kernel (t : Kernel.Term.t) =
  ...
  let hd = match t.hd with
    ...
    | Term.Fun (f, tele, t) -> Fun (f, List.map (fun (v, ty, t) -> (v, of_kernel ty, Option.map of_kernel t)) tele, of_kernel t)
    | Term.Type s -> Type s
...
module Context_ = struct
  ...
  let fold_telescope ?(avoid_capture=true) f x tele k ctx =
    let ctx', r = List.fold_left (fun (ctx, x) (v, ty, t) ->
      ...
      fst (push_var ~avoid_capture:false (v, ty, t) ctx), r) (ctx, x) tele in
  ...
  module Monad = struct
    ...
    module Head = struct
      let map ?(avoid_capture=false) f hd = match hd with
        ...
        | Fun (x, tele, body) ->
          fold_telescope ~avoid_capture (fun rtele (v, ty, t) ->
            let* ty = f ty in
        ...
        | Ind (v, a, c) ->
          let* a' = f a in
          let+ c = with_var ~avoid_capture (v, a, None) (List.map f c) in
          Ind (v, a', c)
  ...
  let rec new_evar ?(ty=None) ?(with_ctx=true) =
    ...
    fun ctx ->
      ...
      { ctx with evar = IMap.add n (ty, None, []) ctx.evar}, { hd = Evar n; args = if with_ctx then List.rev (List.init (depth ctx) mkVar) else [] }
...
let rec fold ?(avoid_capture=true) ?(keep_evars=false) fold_hd fold_app t =
  ...
  let** hd, args = match t.hd with
  | Evar i when not keep_evars ->
    let targs = t.args in
    let+* body = Context_.get_evar_body i in
    (match body with | None -> t.hd, t.args | Some t ->
    if not (List.is_empty t.args) then t.hd, t.args @ targs else
    let n = min (match t.hd with | Fun (_, tele, _) -> List.length (List.take_while (fun (_, _, t) -> Option.is_none t) tele) | _ -> 0) (List.length targs) in
    ...
    t.hd, t.args @ rest)
  | _ -> Context_.Monad.iret (t.hd, t.args) in
  let* hd = Context_.Monad.Head.map ~avoid_capture fold hd in
  let* hd = fold_hd hd in
  let* args = Context_.Monad.List.map fold args in
  fold_app (hd :: args)

let print ?(keep_evars=true) ?(debug=false) t =
  let (+) = String.cat in
  let ret = Context_.Monad.ret in
  let+* (t, _) = Context_.Monad.to_imut (fold ~avoid_capture:false ~keep_evars (function
    ...
    | Evar i -> ret ("?" + string_of_int i, true))
    (function
      | [hd] -> ret hd
...
let free_vars =
  ...
  let rec aux k t =
    let hd = match t.hd with
      ...
      | Fun (_, tele, body) ->
        let k, f = List.fold_left (fun (k, f) (_, ty, t) -> Int.(k + 1), f + aux k ty + Option.map_or ISet.empty (aux k) t) (k, ISet.empty) tele in
        f + aux k body
...
let rec eq t t' =
  ...
  | Fun (f, t, b), Fun (f', t', b') -> ret (f = f') &&
    ...
    Context_.Monad.to_imut (Context_.Monad.List.for_all2 (fun (_, ty, t) (_, ty', t') -> Context_.Monad.to_mut (eq ty ty' &&
    match t, t' with
    | None, None -> ret true
  ...
  | Ind (_, a, c), Ind (_, a', c') -> (ret (List.length c = List.length c')) && eq a a' && Context_.Monad.to_imut (Context_.Monad.List.for_all2 (fun t t' -> Context_.Monad.to_mut (eq t t')) c c')
...
let occurs t' t =
  ...
  let rec aux t' t =
    ...
    | Fun (_, tele, body) ->
      let* t' = Context_.Monad.List.fold_left (fun (_, ty, t) t' ->
...
let eta ?(steps=None) t =
  ...
  match t.hd with
  | Fun (false, tele, body) when List.for_all (fun (_, _, t) -> Option.is_none t) tele -> 
...
let rec iota ?(flags=whd_flags_all) t : t option Context_.Monad.it =
  ...
  let** rargs =
    ...
    Context_.Monad.to_imut (Context_.with_var ~avoid_capture:false (v, a, None) (
...
and destArity ?(whd_rty=false) ?(keep_let=false) ?(until=Max) (t : t) : (t telescope * t) Context_.Monad.t =
(*   let** () = let+* t = print t in print_endline ("destArity " ^ (match until with | Max -> "= oo" | Exact n -> "= " ^ string_of_int n | AtMost n -> "<= " ^ string_of_int n) ^ " " ^ t) in *)
  ...
  | Fun (true, tele, body) when keep_let ->
    ...
    let+ tele2, ty = Context_.with_telescope ~avoid_capture:false tele (destArity ~whd_rty ~keep_let ~until body) in
    tele @ tele2, ty
  | Fun (true, tele, body) ->
    ...
    let subst k =
      ...
      subst (fun i -> if n <= i then mkVar (i - k) else
        match Dynarray.get args i with
        | Either.Left t -> t
    ...
    if until_opt until = Some 0 then ret (tele, body) else
    let+ tele2, ty = Context_.with_telescope ~avoid_capture:false tele (destArity ~whd_rty ~keep_let ~until body) in
    tele @ tele2, ty
  | _ when match until with | Exact _ -> false | _ -> true -> ret ([], if whd_rty then t' else t)
  | Evar _ ->
    ...
    tele, ty
  | _ -> fun ctx -> raise (TypeError (ctx, IllegalApplication t))
...
(*   let** () = let+* ty = print ty in print_endline ("hdty = " ^ ty) in *)
...
(*   let** () = let+* ty = print ty in print_endline ("type is " ^ ty) in *)
  ret ty
...
and rm_problematic_term pb t =
  ...
  let fold_hd hd = match hd with
    ...
    | Fun (f, tele, body) ->
      let rtele, b = List.fold_left (fun (rtele, b) (v, ty, t) ->
  ...
  and fold_app hd args =
    ...
    | Fun (false, tele, body) ->
      let ntele =
        ...
        List.length tele - List.length (List.take_while (fun (_, ty, t) -> Either.is_left ty && Option.map_or true Either.is_left t) (List.rev tele)) in
...
(*   let () = print_endline ("prune_evar " ^ ISet.print args) in *)
  ...
  let tele = List.rev rtele in
  let ty = mkForall tele ty in
  let* ev' = Context_.new_evar ~ty:(Some ty) ~with_ctx:false in
  let* b = unify { hd = Evar ev; args = List.rev (List.init (k + 1) mkVar) } (mkApp (List.map (fun i -> mkVar (k - i)) (List.rev rargs)) ev') in
