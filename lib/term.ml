  ...
  let rec mixed ?(cumulative=Conv) (h1, args1 as t1) (h2, args2) =
    ...
    | _, Const (u2, c2) ->
      ...
      | Const (u1, c1) -> Context.Monad.ret (c1 = c2) && (
        ...
        Context.Monad.ret (List.length args2 = n) && (
...
let rec type_of t =
  ...
  | Const (u, c) ->
    ...
    Context.Monad.to_mut (Context.get_const_type c)
  ...
  | App (f :: a) ->
    List.fold_left (fun ty t ->
      ...
      | Pi ((_, ty) :: tele, body) ->
        ...
        let* b = unify tyt ty in
        if b then Context.Monad.ret (beta t (mkPi tele body)) else fun ctx -> raise (TypeError (ctx, TypeMismatch (ty, t)))
